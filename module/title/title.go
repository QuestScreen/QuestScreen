...
func (t *Title) Init(common *module.SceneCommon) error {
	if len(common.Fonts) == 0 {
		return errors.New("no fonts loaded")
	}
	t.common = common
	t.curTitle = nil
	t.curFont = data.SelectableFont{Family: common.Fonts[0].Name,
...
func (t *Title) InitTransition() time.Duration {
	...
	face := t.common.GetFontFace(t.config.Font)
...
func (t *Title) DefaultConfig() interface{} {
	return &titleConfig{Font: &data.SelectableFont{
		Family: t.common.Fonts[0].Name, FamilyIndex: 0, Size: data.HeadingFont,
		Style: data.Bold}}
}
